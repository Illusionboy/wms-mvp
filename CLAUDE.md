...
│   ├── system_log.py             # Generic anomaly/event log (negative_stock, allocation_conflict, ...)
...
│   └── system_log.py       # write_system_log / get_system_logs — generic anomaly log
├── api/
...
│           ├── analytics.py       # Read-only history/analytics: /counterparties /supplier-history /customer-history /product-history /product-summary /system-logs
...
`SystemLog`（`app/models/system_log.py`）是通用异常/事件日志表，不绑定任何单一功能模块（区别于客户预留专用的 `AllocationConflictLog`，见下）。字段：`category`（`negative_stock` / `allocation_conflict`，后续异常类型可继续扩展）、`level`（`warning`/`error`/`info`）、`message`、`jan_code`、`warehouse_name`。

写入入口 `app/services/system_log.write_system_log()`，目前接入两处：

- `stock_out_item` / `adjust_stock_item`（`app/services/inventory.py`）——出库或调整后库存变为负数时写 `negative_stock` 日志（lazy import，避免循环依赖）。
- `_revalidate_for_jan`（`app/services/customer_allocations.py`）——客户预留从 `reserved` 因库存不足被迫降级为 `waiting` 时，除了写结构化的 `AllocationConflictLog`（供客户预留页面的冲突日志面板展示详情）外，同时写一条 `allocation_conflict` 摘要到 `SystemLog`，使其也出现在全局日志页。

新增异常场景只需在对应 service 里 `from app.services.system_log import write_system_log` 后调用即可汇入同一张表、同一个页面，无需另开页面。

查询入口 `GET /api/v1/analytics/system-logs?category=&level=&limit=`（只读、无需认证），对应前端侧边栏「历史查询 → 日志查看」独立页面。
